...
System.register(['lodash', 'jquery', 'jquery.flot', 'jquery.flot.pie', 'd3'], function (_export, _context) {
  "use strict";
  ...
  function link(scope, elem, attrs, ctrl) {
    ...
    function render() {
      ...
      var matrix_svg = d3.select(elem[0]).append('svg').attr('width', width + margin.left + margin.right).attr('height', height + margin.top + margin.bottom).append('g')
      ...
      var y_labels = matrix_svg.selectAll('.yLabel').data(ctrl.matrix.srcs).enter() // Initial join of data, creates an element for each item in data
      .append('text') // Does this for each element in data
      .text(function (d) {
        return d;
      }) // The value should be the data itself
      ...
      var x_labels = matrix_svg.selectAll('.xLabel').data(ctrl.matrix.dsts).enter().append('text').text(function (d) {
        return d;
      })
      ...
      var cells = matrix_svg.selectAll('.grid-cell').data(ctrl.matrix.cells).enter().append('rect')
      // To add text, I'd need a `g` to wrap, and then rect and text in it
      .attr('x', function (d) {
        return d.row * cellWidth;
      }).attr('y', function (d) {
        return d.col * cellHeight;
      }).attr('class', 'grid-cell').attr('width', cellWidth).attr('height', cellHeight).style('fill', function (d) {
        return 'background-color' in d.style ? d.style['background-color'] : 'transparent';
      });
    }
  }
  ...
  return {
    setters: [function (_lodash) {
      ...
      $ = _jquery.default;
    }, function (_jqueryFlot) {}, function (_jqueryFlotPie) {}, function (_d) {
      d3 = _d;
    }],
    execute: function () {}
  };
